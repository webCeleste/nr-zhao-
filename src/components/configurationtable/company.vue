  ...
  <div>
    ...
    >
      ...
      >
        ...
        <el-form-item label="值班单位名称" prop="company">
          <el-input v-model="ruleForm.company" @focus="companyvalue"></el-input>
        </el-form-item>
        <el-form-item label="值班员" prop="beonduty">
...
export default {
  ...
  methods: {
      closetransfer1(){
          ...
          this.ruleForm.beondutyadmin = this.valuelistradio[0].label;    //值班管理员动态获取，以便于生产
      },
    ...
    //移除单选按钮
    dellistradio(){
          this.valuelistradio.map((val) => {
            if (val.id == this.radio) {
              this.datalsitradio.push(val);
            }
          });
          this.valuelistradio.map((val, index) => {
            if (val.id == this.radio) {
              this.valuelistradio.splice(index, 1);
            }
          });
      },
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid && this.code == 0) {
          ...
          this.$emit("ruleForm", params);
          this.$emit("update:tableboole", false);
